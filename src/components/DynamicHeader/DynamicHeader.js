...
class DynamicHeader extends Component {
  ...
  constructJackpot = jackpot => {
      const fontImgBaseURL = "../../assets/Header/jackpot-font";
      ...
      const mappedSymbols = jackpotSymbols.map((symbol, index) => {
          ...
          htmlString = htmlString.concat(
              `<img class="header_jackpot_symbol -number" src="${this.state.symbols[symbol]}" alt="${symbol}"/>`
          );
          return htmlString;
      });

      mappedSymbols.push(`<img class="header_jackpot_symbol -currency" src="${this.state.symbols["euro"]}" alt="euro"/>`)
